...
public class ControllerNoticias {
	...
	@Autowired
	    ...
	    @PostMapping
	    public NoticiasModel criarProduto(@RequestBody NoticiasModel noticias) {
	        return noticiasService.salvar(noticias);
	    }

	    // Atualizar um produto
	    @PutMapping("/{id}")
	    public ResponseEntity<NoticiasModel> atualizarProduto(@PathVariable Long idNoticias, @RequestBody NoticiasModel noticias) {
	    ...
	    // Excluir um produto
	    @DeleteMapping("/{id}")
	    public ResponseEntity<Void> excluirProduto(@PathVariable Long idNoticias) {
	        Optional<NoticiasModel> noticias = noticiasService.buscarPorId(idNoticias);
